function compute_prf_patient(subjectcode, session, nifti, n_volumes, output_dir, threshold, AnalyzeMergedRuns, AnalyzeSeparateRuns, AnalyzeAveragedRuns, UseDenoise)
%
...
if AnalyzeSeparateRuns == true
    ...
    images_run1 = read_bair_stimuli_patient(subjectcode, session, run, n_volumes, TR);
    ...
    n_volumes = hdr.ImageSize(4);                     % no. of dynamics
    % n_volumes = min(n_volumes, MAX_N_VOLUMES);
    images_run2 = read_bair_stimuli_patient(subjectcode, session, run, n_volumes, TR);
