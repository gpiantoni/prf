 function output_stim = read_bair_stimuli_patient(subjectcode, session, run, n_volumes, TR);
% 
...
% Use stimuli of sub-visual01
sub = 'sub-visual01';

% Read in session corresponding to sub-visual01
...
if run == 1
    PATH_TO_STIMULI = ['/Fridge/R01_BAIR/visual_fmri/data/bids/stimuli/', sub, '_', ses, '_task-bairprf_run-01.mat'];      % First run
    img = load (PATH_TO_STIMULI);
    images = img.stimulus.images;
else
    PATH_TO_STIMULI = ['/Fridge/R01_BAIR/visual_fmri/data/bids/stimuli/', sub, '_', ses, '_task-bairprf_run-02.mat'];      % Second run
    img = load (PATH_TO_STIMULI);
    images = img.stimulus.images;  
end
